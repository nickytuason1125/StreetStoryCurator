...
# ── ONNX image encoder (opt-in: CULLWISE_ENCODER=onnx) ─────────────────────
...
def _onnx_enabled() -> bool:
    ...
    at equal speed. CULLWISE_ENCODER=torch forces the PyTorch path back — the
...
def _onnx_session(graph: str = ""):
    ...
    # rather than failing. CULLWISE_ORT_PROVIDERS overrides the order.
