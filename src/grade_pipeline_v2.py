...
def _dedup_chunk_size(n: int) -> int:
    ...
    which is nothing next to the encoder. CULLWISE_DEDUP_CHUNK overrides.
    """
    _env = os.environ.get("CULLWISE_DEDUP_CHUNK")
    if _env:
        try:
...
def _iqa_resumable(
    ...
    CULLWISE_IQA_SLICE tunes it (0 disables slicing entirely).
    """
    ...
    try:
        _slice = int(os.environ.get("CULLWISE_IQA_SLICE", "400"))
    except ValueError:
        _slice = 400
...
def run_v2(
    ...
    if deep_grade and not scan_mode:
        try:
            ...
            if True:
                ...
                # full VLM look. Disable with CULLWISE_FUNNEL=0.
                _funnel_results: dict = {}
                _vlm_paths = list(paths_to_rate)
                import os as _os_fn
                if _os_fn.environ.get("CULLWISE_FUNNEL", "1") != "0" and len(paths_to_rate) >= 8:
                    try:
                        ...
                        # CULLWISE_FUNNEL=0 or dial CULLWISE_FUNNEL_FRAC=0.
                        _FUNNEL_FRAC = float(_os_fn.environ.get("CULLWISE_FUNNEL_FRAC", "0.35"))
                        _FUNNEL_CEIL = float(_os_fn.environ.get("CULLWISE_FUNNEL_CEIL", "0.50"))
    ...
    # CULLWISE_LUM_DRAFT=0 to go back to full-resolution lum reads.
    _LUM_DRAFT = os.environ.get("CULLWISE_LUM_DRAFT", "1").strip() == "1"
    ...
    # CULLWISE_FUSION_DUMP=<path.npz> writes every per-photo signal that feeds
    ...
    _dump_to = os.environ.get("CULLWISE_FUSION_DUMP", "").strip()
    if _dump_to:
        try:
            np.savez(
                _dump_to,
    ...
    #
    # OPT-IN (CULLWISE_STEP4E=1) as of 2026-06-11: on this 6 GB machine the
    ...
    import os as _os_4e
    if not scan_mode and _os_4e.environ.get("CULLWISE_STEP4E", "0") == "1":
        try:
    ...
    # ── Step 5: PersonalHead adjustment (OPT-IN, off by default) ─────────────
    # 2026-08-30 product decision: star ratings are NOT ground truth for the
    ...
    # CULLWISE_PERSONAL_TASTE=1; otherwise grades stand on the grader alone
    ...
    _taste_on    = os.environ.get("CULLWISE_PERSONAL_TASTE", "").strip() == "1"
    if _ph_weights.exists() and not _taste_on:
        print("[v2] PersonalHead weights present but the taste blend is OPT-IN "
              "(set CULLWISE_PERSONAL_TASTE=1 to enable) — ratings never "
    ...
    if _ph_weights.exists() and _taste_on:
        ...
        try:
            ...
            _env_cap = os.environ.get("CULLWISE_PH_WEIGHT_MAX", "").strip()
            if _env_cap:
            ...
            traceback.print_exc()
            _dump_locals(locals())
    elif not _ph_weights.exists():
        print("[v2] PersonalHead weights absent — using raw grader scores")

    # ── Step 5a: MasterJudge blend — two-phase master-algo contract ───────────
    ...
    #   (CULLWISE_MASTER_JUDGE=1) — user ratings must never grade.
    ...
    # CULLWISE_MASTER_JUDGE_OFF=1 disables everything.
    if not os.environ.get("CULLWISE_MASTER_JUDGE_OFF", "").strip():
        try:
            import master_judge as _mj
            _mj_judge, _mj_w = _mj.active()
            if _mj_w > 0.0 and per_photo_breakdowns:
                _mj_scores = _mj.predict_many(per_photo_breakdowns, final_scores,
                                              weights=_mj_judge)
                _mj_ok = np.isfinite(_mj_scores)
                if _mj_ok.any():
                    final_scores[_mj_ok] = ((1.0 - _mj_w) * final_scores[_mj_ok]
                                            + _mj_w * _mj_scores[_mj_ok])
                    print(f"[v2] MasterJudge blend ({_mj_judge.get('_source')}): "
                          f"weight {_mj_w:.2f} → "
                          f"{int(_mj_ok.sum())}/{n} images "
                          f"(rho {_mj_judge.get('rho_holdout')} vs baseline "
                          f"{_mj_judge.get('rho_baseline')} on held-out master photos)")
        except Exception as _mj_err:
            print(f"[v2] MasterJudge blend skipped ({_mj_err})")
    ...
    try:
        ...
        _LANCE_CHUNK = int(os.environ.get("CULLWISE_LANCE_CHUNK", "500"))
        _written = 0
...
if __name__ == "__main__":
    ...
    _parser = argparse.ArgumentParser(
        description="Cullwise — vision grading pipeline",
    ...
    print("=" * 62)
    print(f"  Cullwise — pipeline test run")
