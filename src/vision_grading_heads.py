...
def _iqa_chunk_size(n: int) -> int:
    ...
    already handled. FIRSTCUT_IQA_CHUNK overrides for a memory-tight machine —
    at the cost of reproducibility across machines with different free RAM.
    """
    import os as _os
    _env = _os.environ.get("FIRSTCUT_IQA_CHUNK")
    if _env:
        try:
...
def _run_vision_heads_streaming(
    ...
    Enable with FIRSTCUT_SHARED_DECODE=1.
    """
    import os as _os
    import time as _t
    ...
    try:
        chunk = int(_os.environ.get("FIRSTCUT_STREAM_CHUNK", "128"))
    except ValueError:
        chunk = 128
...
def run_vision_heads(
    ...
    """
    import os as _os_sd
    if _os_sd.environ.get("FIRSTCUT_SHARED_DECODE", "").strip() == "1":
