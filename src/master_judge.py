...
def load(weights_path: "Path | None" = None) -> "dict | None":
    ...
    if os.environ.get("CULLWISE_MASTER_JUDGE_OFF", "").strip():
        return None
...
#     opt-in (CULLWISE_MASTER_JUDGE=1): user ratings must never grade.
...
def active() -> tuple:
    ...
    Precedence, per the two-phase contract:
      1. kill switch CULLWISE_MASTER_JUDGE_OFF=1 → nothing
      2. a local cache challenger won its exam AND the user opted in
         (CULLWISE_MASTER_JUDGE=1) → the local judge — it is newer than
    ...
    if os.environ.get("CULLWISE_MASTER_JUDGE_OFF", "").strip():
        return None, 0.0
    cache = load()
    if cache and os.environ.get("CULLWISE_MASTER_JUDGE", "").strip() == "1":
...
_AUTOFIT_DELTA = max(5, int(os.environ.get("CULLWISE_MASTER_AUTOFIT_EVERY", "25") or 25))
...
def maybe_autofit(n_now: "int | None" = None) -> dict:
    ...
    if os.environ.get("CULLWISE_MASTER_JUDGE", "").strip() != "1":
