...
def _chunk_size(n: int) -> int:
    ...
    trading determinism for. FIRSTCUT_DFINE_CHUNK still overrides.
    """
    import os as _os
    _env = _os.environ.get("FIRSTCUT_DFINE_CHUNK")
    if _env:
        try:
...
# Draft-decode is revertible: FIRSTCUT_DFINE_DRAFT=0 restores full-resolution
