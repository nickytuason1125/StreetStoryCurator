...
def _chunk_size(n: int) -> int:
    ...
    trading determinism for. LUMARA_DFINE_CHUNK still overrides.
    """
    import os as _os
    _env = _os.environ.get("LUMARA_DFINE_CHUNK")
    if _env:
        try:
...
# Draft-decode is revertible: LUMARA_DFINE_DRAFT=0 restores full-resolution
...
def _draft_enabled() -> bool:
    import os as _os
    return _os.environ.get("LUMARA_DFINE_DRAFT", "1").strip() != "0"
