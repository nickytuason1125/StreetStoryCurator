"""
Cullwise — local desktop launcher.
...
_APP_ID = "StreetPhotography.Cullwise.1"
...
def _start_frontend_watch():
    ...
    #
    # Set CULLWISE_FRONTEND_WATCH=1 while developing. `npm run watch` in a
    # terminal does the same job and shows you its output.
    if os.environ.get("CULLWISE_FRONTEND_WATCH", "") not in ("1", "true", "yes"):
        _log("Frontend watch not started (set CULLWISE_FRONTEND_WATCH=1 for dev)")
        return
