...
# grader's job, not the bouncer's. Tunable via LUMARA_BLUR_MIN for the field.
try:
    _BLUR_VAR_MIN = float(_os.environ.get("LUMARA_BLUR_MIN", "4.0"))
...
# Tunable via LUMARA_FLAT_STD.
try:
    _FLAT_STD_MIN = float(_os.environ.get("LUMARA_FLAT_STD", "2.0"))
except (TypeError, ValueError):
    _FLAT_STD_MIN = 2.0
...
def decode_workers(n: int, mb_per_worker: float = 120.0,
    ...
    the full `hi`. Override with LUMARA_GATE_WORKERS.
    """
    _env = _os.environ.get("LUMARA_GATE_WORKERS")
    if _env:
        try:
