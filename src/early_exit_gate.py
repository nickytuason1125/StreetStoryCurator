...
# grader's job, not the bouncer's. Tunable via FIRSTCUT_BLUR_MIN for the field.
try:
    _BLUR_VAR_MIN = float(_os.environ.get("FIRSTCUT_BLUR_MIN", "4.0"))
...
# Tunable via FIRSTCUT_FLAT_STD.
try:
    _FLAT_STD_MIN = float(_os.environ.get("FIRSTCUT_FLAT_STD", "2.0"))
except (TypeError, ValueError):
    _FLAT_STD_MIN = 2.0
...
def decode_workers(n: int, mb_per_worker: float = 120.0,
    ...
    the full `hi`. Override with FIRSTCUT_GATE_WORKERS.
    """
    _env = _os.environ.get("FIRSTCUT_GATE_WORKERS")
    if _env:
        try:
