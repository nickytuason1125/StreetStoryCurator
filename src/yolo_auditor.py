"""
Person auditor — short-circuit gate for empty/liminal scene briefs.

Uses D-FINE-nano (Apache-2.0, src/dfine_detector.py) for person detection —
replaces ultralytics YOLO (AGPL-3.0). Only activates when the active
Creative Direction brief implies empty scenes. Returns the set of image
paths where a person was detected at conf >= threshold.
...
_CONF_THRESH        = 0.35    # strict person detection threshold
...
def audit_paths(
    ...
    detections = dfine_detector.detect_persons(paths, conf=conf)
    ...
    for path in paths:
        try:
            ...
            for det in boxes:
                ...
                if canvas_area and box_area < area_thresh:
                    print(
