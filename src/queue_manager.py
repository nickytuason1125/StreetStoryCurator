...
def _annotate_min_ram_gb() -> float:
    """Free-RAM floor below which annotation waits. FIRSTCUT_ANNOTATE_MIN_RAM_GB."""
    import os
    try:
        return float(os.environ.get("FIRSTCUT_ANNOTATE_MIN_RAM_GB", "3.0"))
...
def _annotation_backend_available() -> bool:
    ...
    start on a machine that had a running Ollama but none of FirstCut's own
