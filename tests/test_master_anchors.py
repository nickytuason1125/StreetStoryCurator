...
preference itself is OPT-IN — without CULLWISE_MASTER_ANCHORS=1 the file
...
def _opt_in(monkeypatch):
    monkeypatch.setenv("CULLWISE_MASTER_ANCHORS", "1")
...
def test_without_opt_in_master_anchors_are_ignored(tmp_path, monkeypatch):
    ...
    not steer the absolute scale unless CULLWISE_MASTER_ANCHORS=1."""
    monkeypatch.delenv("CULLWISE_MASTER_ANCHORS", raising=False)
    pos, neg = _probes()
    fp = sp.probe_fingerprint(pos, neg)
    master = tmp_path / "master_anchors.json"
    lib = tmp_path / "calibration_anchors.json"
