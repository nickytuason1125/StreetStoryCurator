...
def test_requirement_is_higher_without_draft_decode(monkeypatch):
    ...
    import run_profile as rp
    monkeypatch.setenv("FIRSTCUT_DRAFT_DECODE", "0")
    off = rp.required_ram_gb(250)
    monkeypatch.setenv("FIRSTCUT_DRAFT_DECODE", "1")
...
def test_env_override_still_wins():
    """FIRSTCUT_MIN_RAM_GB stays an escape hatch for an unusual machine."""
    import os
    import run_profile as rp
    os.environ["FIRSTCUT_MIN_RAM_GB"] = "1.5"
    try:
        assert rp.required_ram_gb(250) == 1.5
    finally:
        os.environ.pop("FIRSTCUT_MIN_RAM_GB", None)


# ── 7. Rust mirror stays in sync with Python's numbers ───────────────────────
# native/firstcut-rs/src/main.rs::ram_need_gb copies _RAM_NEED_GB verbatim
...
def test_rust_ram_mirror_matches_python():
    ...
    rs_path = _ROOT / "native" / "firstcut-rs" / "src" / "main.rs"
    ...
    assert not mismatches, (
        "native/firstcut-rs/src/main.rs::ram_need_gb has drifted from "
