...
- **Never delete model weights.** The established precedent in this repo is to move them off the working disk with a documented restore path (the RAG source books went to `D:\cullwise_bench\rag_source` on 2026-08-13). A mistake here costs a multi-gigabyte re-download.
...
**Files:**
- Move: `models/qwen3_vl/` → `D:\cullwise_bench\rejected_models\qwen3_vl\`
...
Run: `ls -d /d/cullwise_bench 2>/dev/null && df -h /d | tail -1`
...
```bash
mkdir -p /d/cullwise_bench/rejected_models
mv models/qwen3_vl /d/cullwise_bench/rejected_models/qwen3_vl
```
...
```bash
du -sh /d/cullwise_bench/rejected_models/qwen3_vl && \
find /d/cullwise_bench/rejected_models/qwen3_vl -type f | wc -l && \
...
|---|---|---|---|
| Qwen3-VL-4B | `D:\cullwise_bench\rejected_models\qwen3_vl\` | 8.3 GB | rejected 2026-06-14; rho +0.078 vs the photographer's stars (chance is +0.149) — see MODELS.md |

To restore one: `mv /d/cullwise_bench/rejected_models/qwen3_vl models/qwen3_vl`.
