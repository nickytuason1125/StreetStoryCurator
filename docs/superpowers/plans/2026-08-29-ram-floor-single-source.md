...
- The env overrides `CULLWISE_MIN_RAM_GB` (absolute override, wins when `> 0`) and `CULLWISE_DRAFT_DECODE` (`"0"` disables draft decode, anything else enables) must behave identically in the Python and Rust implementations.
...
**Files:**
- Modify: `native/cullwise-rs/src/main.rs:63-75` (the `system_ram` handler), plus a new function and a new test module
- Test: `native/cullwise-rs/src/main.rs` (`#[cfg(test)] mod tests` at the end of the file)
...
Background: `native/cullwise-rs` is "slice 1" of a native orchestrator whose stated contract is *byte-compatible response shapes, so the React frontend can point at this process without knowing the difference*. Its `/api/system/ram` handler hardcodes `"ram_min_gb": 1.8`. After the Python change that is a live disagreement between two servers claiming to be interchangeable — and it is the shim that is wrong.
...
Append to the end of `native/cullwise-rs/src/main.rs`:
...
Run: `cd native/cullwise-rs && cargo test`
...
In `native/cullwise-rs/src/main.rs`, immediately above the `// ── handlers: telemetry ──` comment (currently around line 61), insert:
...
/// CULLWISE_MIN_RAM_GB (absolute override) and CULLWISE_DRAFT_DECODE
/// ("0" disables scaled decode, which roughly doubles the requirement).
fn required_ram_gb(n_photos: u32) -> f64 {
    let override_gb = std::env::var("CULLWISE_MIN_RAM_GB")
        .ok()
        .and_then(|s| s.trim().parse::<f64>().ok());
    let draft = std::env::var("CULLWISE_DRAFT_DECODE")
        .map(|s| s.trim() != "0")
        .unwrap_or(true);
...
Run: `cd native/cullwise-rs && cargo build 2>&1 | tail -20`
...
```bash
git add native/cullwise-rs/src/main.rs
...
Background: this is roughly 285 lines of measured work — draft decode (JPEG DCT-domain downscaling, 279 ms/img decode against 3 ms of model time), the encoder-floor / whole-cull-budget split, the D-FINE 640-not-512 finding, and the `CULLWISE_LUM_DRAFT` default flip after a verified zero-bucket-change diff. It is complete and tested; it has simply never been committed. Do not restructure it — commit it.
...
If `draft decode on` prints `False`, something has `CULLWISE_DRAFT_DECODE=0` set in the environment and the floors will read 6.6/7.0. Unset it and re-run.
