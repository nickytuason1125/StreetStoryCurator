...
**Date:** 2026-08-03
**Goal:** Cut Cullwise's on-disk footprint from ~24 GB to between ~10 and
...
| | Cullwise | Lightroom Classic 15.4.1 |
|---|---|---|
...
catalogs on this machine. Cullwise's model weights are a *fixed* cost.
...
- **Retention window, default 7 days.** Declared in the existing `SETTINGS`
  table as `CULLWISE_LANCE_RETENTION_DAYS`, not a literal — per the project's
...
Cullwise's ~22 GB remains a fixed cost. That comparison still holds — it was
