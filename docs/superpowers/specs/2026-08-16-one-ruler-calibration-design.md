...
An escape hatch, `FIRSTCUT_ALLOW_UNCALIBRATED`, keeps the old behaviour for
...
`FIRSTCUT_FUSION_DUMP` site, `grade_pipeline_v2.py:2573`), plus the anchor
...
`photos.lance` is 15.5 GB across 780 versions. `FIRSTCUT_LANCE_RETENTION_DAYS`
...
4. `FIRSTCUT_ALLOW_UNCALIBRATED=1` restores the fallback and logs.
